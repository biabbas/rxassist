...
def diagnosis(request):
    if request.method == "POST":
        return render("patient/chats.html")
...
def send(request):
    ...
    with connection.cursor() as cursor:
        try:
            if user.is_patient: 
                cursor.execute("INSERT INTO chats (diagnosis_id, sender, message) VALUES (%s, %s, %s)", (diag_id, 0, message))
            else:
                cursor.execute("INSERT INTO chats (diagnosis_id, sender, message) VALUES (%s, %s, %s)", (diag_id, 1, message))
...
def conversation(request):
    diag_id = request.POST.get('diag_id')
    user = request.user
    print("Diagnosis id for conversations",diag_id)
    user = request.user
    user_id = user.id
    print(user.id)
    c = connection.cursor()
    c.execute("select sender,message from chats where diagnosis_id = %s order by id", [diag_id])
    messages= c.fetchall()
    if user.is_doctor:
        c.execute("select symptom, disease, medicine from patient_diagnosis join diagnosis_symptoms ON id=patient_diagnosis_id where id=%s",[diag_id])
        details = c.fetchall()
    else:
        details = " "
    context = {"messages": messages,"details": details, "diagnosis_id": diag_id }
    if user.is_patient:
        return render(request, 'patient/chats.html', context)
    else:
        return render(request, 'doctor/chats.html', context)
